...
import { TrackingRetentionService } from '../../tracking/services/tracking-retention.service';
...
export class GpsOfflineProcessor extends WorkerHost {
  ...
  async process(
    job: Job,
  ): Promise<{ created?: number; companies?: number; deleted?: number }> {
    ...
    if (job.name === 'tracking-retention-cleanup') {
      const result = await this.retentionService.purgeExpiredHistory();
      return result;
    }
